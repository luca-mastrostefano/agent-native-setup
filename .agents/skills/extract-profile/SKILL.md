...
- Meta files at the profile root (never shipped): a README.md that opens the way `profile
  ...
  **Description** of what the profile sets up and for whom
  ...
  git+<manager> agent-native-setup -o ./my-app --profile <name>` (a bare name matching nothing
  on disk falls back to the community index, once published) — that's the repo's landing page
  on the registry. Below a `---`, the maintainer half: what's reusable vs adapted and
