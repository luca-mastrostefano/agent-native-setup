...
repo. It reads a `WizardConfig` and scaffolds the **flagship profile**
(`agent-native-baseline`, fetched by pin — RFC 2026-07-08) through the profile pipeline (RFC 2026-07-05 stage B); the
...
| `profiles/baseline-pin.json` | The **pin** for the flagship profile (RFC 2026-07-08; canonical home: [its own repo](https://github.com/luca-mastrostefano/agent-native-baseline)) — tag + `content_hash` + `transient` list. **What `cli.main` scaffolds by default** (`builtin:agent-native-baseline`) is *fetched* from the pinned URL and hash-verified at runtime; the wheel ships this pin, **not** the baseline's bytes. A byte-identical fixture (`tests/fixtures/agent-native-baseline/`, out of the wheel) keeps the suite hermetic; `build.py` there derives its templates from the generators (source of truth until stage D); `tests/test_flagship_parity.py` gates whole-tree byte parity + the offline fixture↔pin match, and `check_baseline_pin.py` / the `index-check` workflow verify the live tag against the pin. |
| `generators/` | One module per concern — `ai_context` (AGENTS.md + per-tool entry points: CLAUDE.md/GEMINI.md symlinks, Cursor/Copilot pointer files, plus a self-removing first-run banner — not optional, RFC 2026-07-09; injected whenever a tool is targeted and an `ONBOARDING.md` ships), `agents` (`.claude/`), `docs` (docs tree + RFC lifecycle + embedded check scripts), `quality` (pre-commit, Taskfile, gitignore), `ci` (GitHub Actions), `onboarding` (the self-deleting `ONBOARDING.md` first-run runbook plus its per-tool transient `/onboard` triggers — Claude/Cursor/Copilot/Gemini, RFC 2026-07-07 — whose last step removes the banner and every trigger). Each exposes `generate(config, sc)`; `agents` and `onboarding` also accept a profile's `session_start`/`onboarding` contributions to merge in. |
| `tools/checks/` | Standalone enforcement scripts run as hooks: `sync_rfc_status.py` (any project with docs), `rfc_needed.py` (any language with a dependency manifest — its dep trigger reads pyproject.toml/package.json/go.mod/Cargo.toml), `docs_sync.py` and `tests_needed.py` (Python `src/`+`tests/` layout), and `format_on_edit.py` (the Claude PostToolUse format-on-edit helper); `check_index.py` (repo-only, not embedded) fetches + validates every community-index entry — run daily / on `contributions/` PRs by the `index-check` workflow. Each ships with a stdlib-`unittest` test, run via `python3 -m unittest discover -s tools/checks` (wired into the command surface, a pre-push hook, and CI). The wizard embeds the scripts and their tests as constants in `generators/docs.py` (gates) and `generators/agents.py` (format hook). |
