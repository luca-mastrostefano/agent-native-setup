...
| `profiles/agent-native-baseline/` | The flagship profile under stage-A extraction (RFC 2026-07-05): the default scaffold being re-expressed as a normal profile. `build.py` derives verbatim templates from the generators' constants (which remain the source of truth); `tests/test_flagship_parity.py` gates byte parity incrementally. See its [README](../../profiles/agent-native-baseline/README.md). |
