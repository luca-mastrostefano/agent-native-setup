...
1. **`.claude/agents/rfc-reviewer.md`** — a read-only agent (Claude, with docs) that
   ...
   overlap an existing active/superseded/retired RFC.
...
- **A "parallel"/always-on reviewer.** An RFC is reviewed once, at draft or before it
  goes Active — not continuously on every change; the `/rfc` flow is the right trigger.
