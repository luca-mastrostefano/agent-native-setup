...
| `profiles.py` | Profiles (RFC 2026-06-23, Phase 1): load/resolve a profile (a dir with `profile.json` + `templates/`) and **overlay** it on the default scaffold as seed files (`extends: default`). Also the `profile init`/`list` authoring CLI. `cli.build` applies the resolved profile before the manifest; `Scaffolder.overlay` does the child-last write. |
