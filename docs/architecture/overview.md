...
| `scaffold.py` | `Scaffolder` — writes files under the target, records what it created and snapshots what `--force` overwrote (for rollback), and renders Jinja templates. Knows nothing about the generators. |
...
- **`config`, `languages`, `pins`, and `scaffold` are leaves** — they don't import the
